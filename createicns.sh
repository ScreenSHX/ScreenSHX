...
if [ -e "$iconset_path" ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "There is a pre-existing file/dir $iconset_path that could not be deleted"
        exit 1
    fi
fi
...
for a in "${icon_file_list[@]}"; do
    ...
    current_icon_size=${icon_sizes[$counter]}
    /usr/bin/sips -z $current_icon_size $current_icon_size "$icon"
    if [ $? -ne 0 ]; then
        echo "There was an error resizing the icon: $icon"
        exit 1
    fi
    counter=$((counter + 1))
done
